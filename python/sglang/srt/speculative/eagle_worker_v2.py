...
class EagleDraftWorker(BaseDraftWorker):
    ...
    def draft_forward(self, forward_batch: ForwardBatch):
        ...
        debug_print(f"[DEBUG draft_forward] batch_size={forward_batch.batch_size}, topk={self.topk}, num_steps={self.speculative_num_steps}")
        debug_print(f"[DEBUG draft_forward] out_cache_loc (flat) shape={out_cache_loc.shape}")
        debug_print(f"[DEBUG draft_forward] out_cache_loc values[:20]={out_cache_loc[:min(20, len(out_cache_loc))]}")

        out_cache_loc = out_cache_loc.reshape(
            forward_batch.batch_size, self.topk, self.speculative_num_steps
        )
        debug_print(f"[DEBUG draft_forward] After reshape to (bs, topk, steps): shape={out_cache_loc.shape}")
        if forward_batch.batch_size > 0:
            debug_print(f"[DEBUG draft_forward] out_cache_loc[0] (all topk/steps for seq 0):\n{out_cache_loc[0]}")
        
        out_cache_loc = out_cache_loc.permute((2, 0, 1)).reshape(
            self.speculative_num_steps, -1
        )
        debug_print(f"[DEBUG draft_forward] After permute to (steps, bs*topk): shape={out_cache_loc.shape}")
        debug_print(f"[DEBUG draft_forward] out_cache_loc[0] (step 0, all bs*topk)={out_cache_loc[0]}")
        if out_cache_loc.shape[0] > 1:
            debug_print(f"[DEBUG draft_forward] out_cache_loc[1] (step 1, all bs*topk)={out_cache_loc[1]}")
        ...
        scores = None
        debug_print(f"[DEBUG draft_forward] Starting {self.speculative_num_steps} forward steps\n")
        for i in range(self.speculative_num_steps):
            ...
            debug_print(f"[DEBUG draft_forward] Step {i}: input_ids (selected tokens)={input_ids}")
            ...
            debug_print(f"[DEBUG draft_forward] Step {i}: out_cache_loc[{i}] (where to store KV)={out_cache_loc[i]}")
            forward_batch.positions.add_(1)
            debug_print(f"[DEBUG draft_forward] Step {i}: positions={forward_batch.positions}")
            ...
            debug_print(f"[DEBUG draft_forward] Step {i}: After forward, topk_index={topk_index}")
